...
lang_dists_by_region <- languages_respno %>%
  filter(variable %in% c("interview", "respondent")) %>%
  filter(!is.na(glottocode)) %>%
  count(round, variable, country, region, glottocode, wt = withinwt) %>%
  group_by(round, variable, country, region) %>%
  mutate(p = n / sum(n),
...
elf_by_region_2 <- lang_dists_by_region %>%
  mutate(d = list(seq_len(max_depth))) %>%
  unnest() %>%
  ungroup() %>%
  mutate(dist = p * p_to * (shared < d)) %>%
  group_by(round, variable, country, region, d) %>%
  summarise(dist = sum(dist)) %>%
  mutate(d = sprintf("elf_d%02d", d)) %>%
  spread(d, dist)

elf_by_region <- left_join(elf_by_region, elf_by_region_2,
                            by = c("round", "variable", "region"))
